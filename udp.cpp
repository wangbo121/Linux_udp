...
int open_socket_udp_dev(int *ptr_fd_socket, char* ip, unsigned int port)
{
    ...
    {
        printf("udp_sendto_addr.sin_addr.s_addr = INADDR_ANY;//INADDR_ANY是任意地址\n");
...
int send_socket_udp_data(int fd_send_socket, unsigned char *buf, unsigned int len, char *target_ip, unsigned int target_port)
{
    ...
    //printf("send len=%d send_buf = %s\n",send_len,send_buf);

    //printf("fd_send_socket = %d\n",fd_send_socket);
...
int open_udp_dev(char* ip_sendto, unsigned int port_sendto, unsigned int port_myrecv)
{
    ...
    struct T_UDP_DEVICE *ptr_udp_device;
    ptr_udp_device=&udp_device;

    unsigned long int *ptr_udp_fd;
    unsigned long int udp_fd_temp;
    udp_fd_temp = (unsigned long int)udp_device.fd_socket;
    ptr_udp_fd=&udp_fd_temp;
    /*udp_recvbuf_and_process函数中调用了fd_sock_recv来确定从哪个socket获取数据*/
    ret = pthread_create (ptr_udp_fd,            //线程标识符指针
                          NULL,                            //默认属性
                          udp_recvbuf_and_process,//运行函数
                          (void *)(ptr_udp_device));                 //运行函数的参数
    if (0 != ret)
    {
...
void *udp_recvbuf_and_process(void * ptr_udp_device)
{
    ...
    //printf("process  sizeof(unsigned long int) = %d\n",sizeof(unsigned long int));
    //printf("process  udp.fd_socket = %d\n",udp.fd_socket);
    //printf("process  udp.fd_receive_thread = %lu\n",udp.fd_receive_thread);
    ...
    while(1)
    {
        ...
        //if(-1!=(read_len=recvfrom(4, buf, 2000, 0, (struct sockaddr *)&from_addr, &from_addr_len)))
        {
            ...
            printf("temp = %s\n",temp);
            printf("udp.ip = %s\n",udp.ip);
            if(strcmp(udp.ip,temp) == 0)
            {
                printf("收到的数据的 发送放方ip地址与我们所希望的ip地址一致，准备接收\n");
            }
            else
            {
                printf("收到的ip地址不正确，不接收\n");
            }
